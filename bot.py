...
IQ_ANSWERS = [
    {"correct": 2, "weight": 1}, {"correct": 0, "weight": 1},
    {"correct": 3, "weight": 1}, {"correct": 2, "weight": 1},
    {"correct": 3, "weight": 1}, {"correct": 1, "weight": 1},
    {"correct": 1, "weight": 2}, {"correct": 2, "weight": 2},
    {"correct": 1, "weight": 2}, {"correct": 1, "weight": 2},
    {"correct": 1, "weight": 2}, {"correct": 2, "weight": 2},
    {"correct": 1, "weight": 3}, {"correct": 0, "weight": 3},
    {"correct": 1, "weight": 3}, {"correct": 1, "weight": 3},
    {"correct": 2, "weight": 3}, {"correct": 1, "weight": 3},
]
EQ_ANSWERS = [
    {"scores": [4,2,3,1]}, {"scores": [4,1,2,1]}, {"scores": [4,3,1,1]},
    {"scores": [4,2,1,2]}, {"scores": [4,2,3,1]}, {"scores": [4,2,1,2]},
]
PQ_ANSWERS = [
    {"scores": [2,1,2,1]}, {"scores": [4,2,1,0]}, {"scores": [4,1,2,2]},
    {"scores": [4,3,1,0]}, {"scores": [4,2,1,0]}, {"scores": [4,3,1,0]},
]
...
async def init_db(pool):
    async with pool.acquire() as conn:
        await conn.execute("""CREATE TABLE IF NOT EXISTS users (
            ...
            language TEXT DEFAULT 'uz', gender TEXT, age INTEGER,
        ...
        await conn.execute("""CREATE TABLE IF NOT EXISTS test_sessions (
            id SERIAL PRIMARY KEY, session_id TEXT UNIQUE NOT NULL,
            user_id BIGINT NOT NULL, test_type TEXT NOT NULL,
            status TEXT DEFAULT 'active', started_at TIMESTAMPTZ DEFAULT NOW(),
            expires_at TIMESTAMPTZ)""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS test_attempts (
            id SERIAL PRIMARY KEY, user_id BIGINT NOT NULL,
            test_type TEXT NOT NULL, session_id TEXT,
            started_at TIMESTAMPTZ DEFAULT NOW(), finished_at TIMESTAMPTZ,
            score INTEGER, correct_count INTEGER, duration INTEGER,
            status TEXT DEFAULT 'in_progress',
            payment_status TEXT DEFAULT 'free',
            result_visible BOOLEAN DEFAULT TRUE, level TEXT)""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS results (
            id SERIAL PRIMARY KEY, user_id BIGINT NOT NULL,
            attempt_id INTEGER NOT NULL, test_type TEXT NOT NULL,
            score INTEGER NOT NULL, level TEXT,
            created_at TIMESTAMPTZ DEFAULT NOW())""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS payments (
            id SERIAL PRIMARY KEY, payment_id TEXT UNIQUE NOT NULL,
            user_id BIGINT NOT NULL, product TEXT NOT NULL,
            amount INTEGER NOT NULL, status TEXT DEFAULT 'pending',
            receipt_file_id TEXT, attempt_id INTEGER, battle_id INTEGER,
            created_at TIMESTAMPTZ DEFAULT NOW(), approved_at TIMESTAMPTZ)""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS payment_cards (
            id SERIAL PRIMARY KEY, card_number TEXT NOT NULL,
            holder TEXT NOT NULL, bank TEXT, active BOOLEAN DEFAULT TRUE,
            created_at TIMESTAMPTZ DEFAULT NOW())""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS certificates (
            id SERIAL PRIMARY KEY, certificate_id TEXT UNIQUE NOT NULL,
            verification_code TEXT UNIQUE NOT NULL,
            user_id BIGINT NOT NULL, result_id INTEGER NOT NULL,
            score INTEGER NOT NULL, type TEXT DEFAULT 'iq',
            full_name TEXT, created_at TIMESTAMPTZ DEFAULT NOW())""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS battles (
            id SERIAL PRIMARY KEY, battle_code TEXT UNIQUE NOT NULL,
            creator_id BIGINT NOT NULL, opponent_id BIGINT,
            status TEXT DEFAULT 'waiting_for_player', winner_id BIGINT,
            created_at TIMESTAMPTZ DEFAULT NOW(), finished_at TIMESTAMPTZ)""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS battle_players (
            id SERIAL PRIMARY KEY, battle_id INTEGER NOT NULL,
            user_id BIGINT NOT NULL, payment_status TEXT DEFAULT 'pending',
            test_status TEXT DEFAULT 'not_started', session_id TEXT,
            current_question INTEGER DEFAULT 0, answers JSONB,
            score INTEGER, weighted INTEGER, finished_at TIMESTAMPTZ,
            joined_at TIMESTAMPTZ DEFAULT NOW(),
            UNIQUE(battle_id, user_id))""")
        await conn.execute("""CREATE TABLE IF NOT EXISTS referrals (
            id SERIAL PRIMARY KEY, referrer_id BIGINT NOT NULL,
            referred_id BIGINT NOT NULL, created_at TIMESTAMPTZ DEFAULT NOW(),
            UNIQUE(referred_id))""")
        ...
        defaults = {
            "iq_price": "0", "iq_retry_price": "5000",
            "eq_price": "0", "eq_retry_price": "5000",
            "pq_price": "0", "pq_retry_price": "5000",
            "battle_price": "7500", "support_username": "omono_v",
            "live_mode": "fake", "live_fake_base": "95114",
            "live_fake_online": "342", "live_fake_delta": "8",
            "live_real_online_minutes": "5",
        }
        for k, v in defaults.items():
            await conn.execute("INSERT INTO app_settings (key, value) VALUES ($1, $2) ON CONFLICT (key) DO NOTHING", k, v)
        if ADMIN_USER_ID:
            await conn.execute("INSERT INTO admins (user_id) VALUES ($1) ON CONFLICT DO NOTHING", ADMIN_USER_ID)
        cnt = await conn.fetchval("SELECT COUNT(*) FROM payment_cards")
        if cnt == 0:
            await conn.execute("INSERT INTO payment_cards (card_number, holder, bank, active) VALUES ('8600 1234 5678 9012', 'IQ TEST BOT', 'Click', TRUE)")
...
async def set_setting(key, value):
    async with db_pool.acquire() as conn:
        await conn.execute("INSERT INTO app_settings (key, value, updated_at) VALUES ($1, $2, NOW()) ON CONFLICT (key) DO UPDATE SET value=$2, updated_at=NOW()", key, str(value))
...
async def upsert_user(user):
    async with db_pool.acquire() as conn:
        await conn.execute("""INSERT INTO users (user_id, username, first_name, last_name, last_seen)
...
async def cmd_start(message: types.Message):
    try:
        ...
        if len(args) > 1 and args[1].startswith("ref_"):
            try:
                ...
                if ref_id != message.from_user.id:
                    async with db_pool.acquire() as conn:
                        await conn.execute("""INSERT INTO referrals (referrer_id, referred_id)
        ...
        if not user or not user["language"]:
            ...
            return
        lang = user["language"]
        await message.answer(t(lang, "welcome", name=message.from_user.first_name or "do‘stim"),
...
async def cb_lang(cb: CallbackQuery):
    try:
        ...
        await cb.message.answer(t(lang, "welcome", name=cb.from_user.first_name or "do‘stim"),
            reply_markup=main_menu_kb(lang))
    except Exception as e:
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            cert = await conn.fetchrow("""
                ...
                FROM certificates c WHERE c.user_id=$1 AND LOWER(TRIM(c.type))='iq'
                ORDER BY c.created_at DESC LIMIT 1
            """, message.from_user.id)
        if not cert:
            await message.answer(t(lang, "no_cert"))
            return
        name = cert["full_name"] or "User"
        png = generate_iq_certificate_png(name=name, score=cert["score"],
...
async def menu_rank(message: types.Message):
    try:
        ...
        lang = user["language"] if user else "uz"
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("""SELECT u.first_name, u.username, MAX(r.score) as best
...
async def menu_help(message: types.Message):
    try:
        user = await get_user(message.from_user.id)
        lang = user["language"] if user else "uz"
        iq_price = await get_setting("iq_price", "0")
...
def generate_iq_certificate_png(name, score, code, date):
    ...
    level = "JUDA YUQORI" if score >= 130 else "YUQORI DARAJA" if score >= 115 else "O‘RTA DARAJA" if score >= 100 else "RIVOJLANTIRISH KERAK"
...
async def lifespan(app: FastAPI):
    ...
    try:
        await bot.set_webhook(url=f"{PUBLIC_BASE_URL}/telegram/webhook",
...
async def api_me(request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not u:
            await conn.execute("""INSERT INTO users (user_id, username, first_name, last_name)
                VALUES ($1, $2, $3, $4)""", uid, user.get("username"), user.get("first_name"), user.get("last_name"))
            u = await conn.fetchrow("SELECT * FROM users WHERE user_id=$1", uid)
        results = await conn.fetch("""SELECT LOWER(TRIM(test_type)) as tt, MAX(score) as best
            FROM results WHERE user_id=$1 AND LOWER(TRIM(test_type)) IN ('iq','eq','pq')
            GROUP BY LOWER(TRIM(test_type))""", uid)
        pending = await conn.fetch("SELECT payment_id, product, amount, status FROM payments WHERE user_id=$1 AND status='pending'", uid)
        battles = await conn.fetch("""SELECT b.id, b.battle_code, b.status FROM battles b
            JOIN battle_players bp ON bp.battle_id = b.id
            WHERE bp.user_id=$1 AND b.status NOT IN ('completed','draw','cancelled')""", uid)
    completed = {r["tt"]: r["best"] for r in results}
    logger.warning(f"API_ME uid={uid} completed={completed}")
    return {
        "ok": True,
        "user": {"user_id": uid, "first_name": user.get("first_name"),
        ...
        "active_battles": [dict(b) for b in battles],
    }
...
async def api_profile_save(request: Request):
    ...
    async with db_pool.acquire() as conn:
        await conn.execute("""UPDATE users SET full_name=$1, gender=$2, age=$3, country=$4
            WHERE user_id=$5""", full_name, gender, age, country, user["id"])
    return {"ok": True}
...
async def api_stats_live():
    ...
    if mode == "real":
        async with db_pool.acquire() as conn:
            ...
            online = await conn.fetchval(f"SELECT COUNT(*) FROM users WHERE last_seen > NOW() - INTERVAL '{minutes} minutes'") or 0
    else:
...
async def api_session_start(request: Request):
    ...
    sid = gen_session_id()
    async with db_pool.acquire() as conn:
        await conn.execute("""INSERT INTO test_sessions (session_id, user_id, test_type, expires_at)
            VALUES ($1, $2, $3, NOW() + INTERVAL '2 hours')""", sid, user["id"], test_type)
        attempt = await conn.fetchrow("""INSERT INTO test_attempts (user_id, test_type, session_id)
            VALUES ($1, $2, $3) RETURNING id""", user["id"], test_type, sid)
...
async def api_test_submit(request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        if test_type == "iq":
            ...
            iq_price = await get_setting_int("iq_price", 0)
            if iq_price > 0:
                payment_status = "pending"; result_visible = False
        elif test_type == "eq":
            total = 0
            ...
            if pq_price > 0:
                payment_status = "pending"; result_visible = False
        await conn.execute("""UPDATE test_attempts
            SET finished_at=NOW(), score=$1, correct_count=$2, duration=$3,
                status='completed', payment_status=$4, result_visible=$5, level=$6
            WHERE id=$7""", score, correct, duration, payment_status, result_visible, level, attempt["id"])
        await conn.execute("UPDATE test_sessions SET status='completed' WHERE session_id=$1", sid)
        res = await conn.fetchrow("""INSERT INTO results (user_id, attempt_id, test_type, score, level)
            VALUES ($1, $2, $3, $4, $5) RETURNING id""", user["id"], attempt["id"], test_type, score, level)
        certificate_code = None
        if test_type == "iq" and result_visible:
            urow = await conn.fetchrow("SELECT full_name, first_name FROM users WHERE user_id=$1", user["id"])
            nm = (urow["full_name"] or urow["first_name"] or "User") if urow else "User"
            certificate_code = gen_code("IQ")
            await conn.execute("""INSERT INTO certificates (certificate_id, verification_code, user_id, result_id, score, type, full_name)
                VALUES ($1, $2, $3, $4, $5, 'iq', $6)""",
                gen_code("CERT"), certificate_code, user["id"], res["id"], score, nm)
    return {"ok": True, "score": score, "correct": correct, "total": len(answers),
        "level": level, "weighted": weighted, "result_visible": result_visible,
        "payment_required": payment_status == "pending",
        "attempt_id": attempt["id"], "certificate_code": certificate_code}

@app.post("/api/certificate/generate")
async def api_certificate_generate(request: Request):
    user, body = await require_user(request)
    async with db_pool.acquire() as conn:
        cert = await conn.fetchrow("""SELECT c.*, u.full_name, u.first_name
            FROM certificates c JOIN users u ON u.user_id = c.user_id
            WHERE c.user_id=$1 AND c.type='iq'
            ORDER BY c.created_at DESC LIMIT 1""", user["id"])
    if not cert:
...
async def api_battle_create(request: Request):
    ...
    async with db_pool.acquire() as conn:
        existing = await conn.fetchrow("""SELECT b.id, b.battle_code FROM battles b
            JOIN battle_players bp ON bp.battle_id = b.id
            WHERE bp.user_id=$1 AND b.status NOT IN ('completed','draw','cancelled')""", user["id"])
        if existing:
        ...
        for _ in range(20):
            ...
            if not chk: break
            code = gen_battle_code()
        b = await conn.fetchrow("""INSERT INTO battles (battle_code, creator_id, status)
            VALUES ($1, $2, 'waiting_for_player') RETURNING id""", code, user["id"])
        await conn.execute("""INSERT INTO battle_players (battle_id, user_id, payment_status, test_status)
            VALUES ($1, $2, 'pending', 'not_started')""", b["id"], user["id"])
...
async def api_battle_join(request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not b: return {"ok": False, "error": "NOT_FOUND"}
        if b["creator_id"] == user["id"]: return {"ok": False, "error": "OWN_BATTLE"}
        if b["status"] not in ("waiting_for_player",): return {"ok": False, "error": "BATTLE_NOT_OPEN"}
        if b["opponent_id"]: return {"ok": False, "error": "BATTLE_FULL"}
        existing = await conn.fetchrow("""SELECT b2.id FROM battles b2
            JOIN battle_players bp ON bp.battle_id = b2.id
            WHERE bp.user_id=$1 AND b2.status NOT IN ('completed','draw','cancelled')""", user["id"])
        if existing: return {"ok": False, "error": "ACTIVE_BATTLE_EXISTS"}
        await conn.execute("UPDATE battles SET opponent_id=$1, status='waiting_for_payment' WHERE id=$2", user["id"], b["id"])
        await conn.execute("""INSERT INTO battle_players (battle_id, user_id, payment_status, test_status)
            VALUES ($1, $2, 'pending', 'not_started') ON CONFLICT (battle_id, user_id) DO NOTHING""", b["id"], user["id"])
...
async def api_battle_get(battle_id: int, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        players = await conn.fetch("""SELECT bp.user_id, bp.payment_status, bp.test_status,
            bp.score, bp.current_question, u.first_name, u.username, u.full_name
            FROM battle_players bp JOIN users u ON u.user_id = bp.user_id
            WHERE bp.battle_id=$1""", battle_id)
        is_member = any(p["user_id"] == user["id"] for p in players)
        if not is_member: raise HTTPException(403, "Not member")
    out = []
    for p in players:
        item = {"user_id": p["user_id"], "payment_status": p["payment_status"],
            "test_status": p["test_status"], "current_question": p["current_question"] or 0,
            "name": p["full_name"] or p["first_name"] or p["username"] or "User",
            "is_me": p["user_id"] == user["id"]}
        if p["test_status"] == "completed" and b["status"] in ("completed", "draw"):
            item["score"] = p["score"]
        out.append(item)
    return {"ok": True, "battle": {"id": b["id"], "code": b["battle_code"],
        "status": b["status"], "creator_id": b["creator_id"],
        "opponent_id": b["opponent_id"], "winner_id": b["winner_id"]}, "players": out}
...
async def api_battle_sync(battle_id: int, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        if not bp: raise HTTPException(403, "Not member")
        await conn.execute("""UPDATE battle_players SET current_question=$1, answers=$2::jsonb, test_status='in_progress'
            WHERE battle_id=$3 AND user_id=$4""", current, json.dumps(answers), battle_id, user["id"])
    return {"ok": True}
...
async def api_battle_start(battle_id: int, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        if len(players) < 2: return {"ok": False, "error": "WAITING_FOR_PLAYER"}
        ...
        if not bp["session_id"]:
            sid = gen_session_id()
            await conn.execute("UPDATE battle_players SET session_id=$1 WHERE battle_id=$2 AND user_id=$3", sid, battle_id, user["id"])
        else:
            sid = bp["session_id"]
...
async def api_battle_finish(battle_id: int, request: Request):
    ...
    async with db_pool.acquire() as conn:
        ...
        bp = await conn.fetchrow("SELECT * FROM battle_players WHERE battle_id=$1 AND user_id=$2", battle_id, user["id"])
        if not bp: raise HTTPException(403, "Not member")
        if bp["test_status"] == "completed": raise HTTPException(409, "Already finished")
        ...
        for i, q in enumerate(IQ_ANSWERS):
            ...
            if ans == q["correct"]:
                weighted += q["weight"]; correct += 1
        score = int(70 + (weighted / max_w) * 60) if max_w else 70
        await conn.execute("""UPDATE battle_players SET score=$1, weighted=$2, answers=$3::jsonb,
            test_status='completed', current_question=18, finished_at=NOW()
            WHERE battle_id=$4 AND user_id=$5""", score, weighted, json.dumps(answers), battle_id, user["id"])
        allp = await conn.fetch("SELECT * FROM battle_players WHERE battle_id=$1", battle_id)
        completed = [p for p in allp if p["test_status"] == "completed"]
        opp_score = None; winner_id = None; final_status = None
        if len(completed) >= 2:
            p1 = completed[0]; p2 = completed[1]
            opp_score = p2["score"] if p1["user_id"] == user["id"] else p1["score"]
            if p1["score"] > p2["score"]: winner_id = p1["user_id"]; final_status = "completed"
            elif p2["score"] > p1["score"]: winner_id = p2["user_id"]; final_status = "completed"
            else: winner_id = None; final_status = "draw"
            await conn.execute("UPDATE battles SET status=$1, winner_id=$2, finished_at=NOW() WHERE id=$3", final_status, winner_id, battle_id)
            if winner_id:
                wbp = next(p for p in completed if p["user_id"] == winner_id)
                urow = await conn.fetchrow("SELECT full_name, first_name FROM users WHERE user_id=$1", winner_id)
                nm = (urow["full_name"] or urow["first_name"] or "User") if urow else "User"
                await conn.execute("""INSERT INTO certificates (certificate_id, verification_code, user_id, result_id, score, type, full_name)
                    VALUES ($1, $2, $3, 0, $4, 'battle', $5)""", gen_code("CERT"), gen_code("BT"), winner_id, wbp["score"], nm)
        else:
            final_status = b["status"]
            await conn.execute("UPDATE battles SET status='player_1_finished' WHERE id=$1", battle_id)
    return {"ok": True, "score": score, "weighted": weighted, "correct": correct,
        "opponent_score": opp_score, "winner_id": winner_id, "final_status": final_status}
...
async def api_payment_create(request: Request):
    ...
    if amount is None: raise HTTPException(400, "Invalid product")
    if amount == 0:
        if product == "iq" and attempt_id:
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE test_attempts SET payment_status='free', result_visible=TRUE WHERE id=$1", attempt_id)
        return {"ok": True, "free": True, "amount": 0}
    async with db_pool.acquire() as conn:
        existing = await conn.fetchrow("""SELECT payment_id FROM payments
            ...
            ORDER BY created_at DESC LIMIT 1""", user["id"], product, attempt_id, battle_id)
        if existing: pid = existing["payment_id"]
        else:
            pid = gen_payment_id()
            await conn.execute("""INSERT INTO payments (payment_id, user_id, product, amount, attempt_id, battle_id)
                VALUES ($1, $2, $3, $4, $5, $6)""", pid, user["id"], product, amount, attempt_id, battle_id)
...
async def api_payment_get(payment_id: str, request: Request):
    user, body = await require_user(request)
    async with db_pool.acquire() as conn:
        p = await conn.fetchrow("SELECT payment_id, product, amount, status FROM payments WHERE payment_id=$1 AND user_id=$2", payment_id, user["id"])
...
async def admin_menu(cb: CallbackQuery):
    ...
    except: await cb.message.answer("👑 <b>ADMIN PANEL</b>", reply_markup=admin_kb())
    await cb.answer()
...
async def admin_stats(cb: CallbackQuery):
    ...
    try:
        ...
        text = f"📊 <b>STATISTICS</b>\n\n👥 Users: <b>{total}</b>\n📅 Today: <b>{today}</b>\n🧠 IQ: <b>{iq}</b>\n🎭 EQ: <b>{eq}</b>\n⏳ PQ: <b>{pq}</b>\n💳 Payments: <b>{pay}</b>\n💰 Revenue: <b>{rev:,} so‘m</b>\n⚔️ Battles: <b>{battles}</b>"
        b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_stats error: {e}")
    await cb.answer()
...
async def admin_users(cb: CallbackQuery):
    ...
    try:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("SELECT user_id, first_name, username, language FROM users ORDER BY created_at DESC LIMIT 20")
        text = "👥 <b>USERS</b>\n\n"
        for r in rows:
            nm = r["first_name"] or r["username"] or "?"
            text += f"• <code>{r['user_id']}</code> — {nm} [{r['language']}]\n"
        b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_users error: {e}")
    await cb.answer()
...
async def admin_payments(cb: CallbackQuery):
    ...
    try:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("""SELECT payment_id, user_id, product, amount, receipt_file_id, created_at
                FROM payments WHERE status='pending' ORDER BY created_at DESC LIMIT 20""")
        if not rows:
            b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
            await cb.message.edit_text("💳 Bo‘sh", reply_markup=b.as_markup()); await cb.answer(); return
        for p in rows:
            bb = InlineKeyboardBuilder()
            bb.button(text="✅", callback_data=f"pay_ok:{p['payment_id']}")
            bb.button(text="❌", callback_data=f"pay_no:{p['payment_id']}")
            bb.adjust(2)
            txt = f"💳 <b>{p['product']}</b>\n👤 <code>{p['user_id']}</code>\n💰 {p['amount']:,} so‘m"
            if p["receipt_file_id"]: await cb.message.answer_photo(p["receipt_file_id"], caption=txt, reply_markup=bb.as_markup())
            else: await cb.message.answer(txt, reply_markup=bb.as_markup())
    except Exception as e: logger.error(f"admin_payments error: {e}")
    await cb.answer()
...
async def pay_ok(cb: CallbackQuery):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            if not p or p["status"] != "pending": await cb.answer("ko‘rib chiqilgan"); return
            await conn.execute("UPDATE payments SET status='approved', approved_at=NOW() WHERE payment_id=$1", pid)
            if p["product"] == "iq" and p["attempt_id"]:
                await conn.execute("UPDATE test_attempts SET payment_status='paid', result_visible=TRUE WHERE id=$1", p["attempt_id"])
                ...
                if att:
                    ...
                    if res:
                        ex = await conn.fetchval("SELECT 1 FROM certificates WHERE result_id=$1 AND type='iq'", res["id"])
                        if not ex:
                            urow = await conn.fetchrow("SELECT full_name, first_name FROM users WHERE user_id=$1", p["user_id"])
                            nm = (urow["full_name"] or urow["first_name"] or "User") if urow else "User"
                            await conn.execute("""INSERT INTO certificates (certificate_id, verification_code, user_id, result_id, score, type, full_name)
                                VALUES ($1, $2, $3, $4, $5, 'iq', $6)""", gen_code("CERT"), gen_code("IQ"), p["user_id"], res["id"], res["score"], nm)
                try: await bot.send_message(p["user_id"], "✅ To‘lov tasdiqlandi!")
                except: pass
            elif p["product"] == "battle" and p["battle_id"]:
                await conn.execute("UPDATE battle_players SET payment_status='approved' WHERE battle_id=$1 AND user_id=$2", p["battle_id"], p["user_id"])
                pls = await conn.fetch("SELECT user_id, payment_status FROM battle_players WHERE battle_id=$1", p["battle_id"])
                if len(pls) >= 2 and all(pl["payment_status"] == "approved" for pl in pls):
                    await conn.execute("UPDATE battles SET status='ready' WHERE id=$1 AND status='waiting_for_payment'", p["battle_id"])
                    for pl in pls:
                        try: await bot.send_message(pl["user_id"], "🎉 Battle tayyor!")
                        except: pass
        if cb.message.caption: await cb.message.edit_caption(caption="✅ Tasdiqlandi")
        else: await cb.message.edit_text("✅ Tasdiqlandi")
    except Exception as e: logger.error(f"pay_ok error: {e}")
    await cb.answer("OK")
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            if p["product"] == "battle" and p["battle_id"]:
                await conn.execute("UPDATE battle_players SET payment_status='rejected' WHERE battle_id=$1 AND user_id=$2", p["battle_id"], p["user_id"])
        if cb.message.caption: await cb.message.edit_caption(caption="❌ Rad etildi")
        else: await cb.message.edit_text("❌ Rad etildi")
    except Exception as e: logger.error(f"pay_no error: {e}")
    await cb.answer("OK")
...
async def admin_products(cb: CallbackQuery):
    ...
    try:
        ...
        b.button(text="⬅️", callback_data="admin:menu")
        b.adjust(2, 2, 2, 1, 1)
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_products error: {e}")
    await cb.answer()
...
async def admin_set_price(cb: CallbackQuery):
    ...
    except Exception as e: logger.error(f"admin_set_price error: {e}")
    await cb.answer()
...
async def admin_cards(cb: CallbackQuery):
    ...
    try:
        async with db_pool.acquire() as conn:
            cards = await conn.fetch("SELECT id, card_number, holder, bank, active FROM payment_cards ORDER BY created_at DESC")
        text = "💳 <b>KARTALAR</b>\n\n"
        for c in cards:
            st = "✅" if c["active"] else "❌"
            text += f"{st} <code>{c['card_number']}</code>\n   {c['holder']} | {c['bank'] or '—'}\n\n"
        if not cards: text += "Bo‘sh"
        b = InlineKeyboardBuilder()
        b.button(text="➕ Yangi", callback_data="card:add")
        for c in cards:
        ...
        b.adjust(1, 2)
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_cards error: {e}")
    await cb.answer()
...
async def card_add(cb: CallbackQuery):
    ...
    await cb.message.edit_text("➕ Format:\n<code>KARTA | HOLDER | BANK</code>")
    await cb.answer()
...
async def card_edit(cb: CallbackQuery):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            c = await conn.fetchrow("SELECT id, card_number, holder, bank FROM payment_cards WHERE id=$1", cid)
        if not c: await cb.answer("Topilmadi"); return
        ADMIN_STATE[cb.from_user.id] = {"action": "card_edit", "id": cid}
        await cb.message.edit_text(f"✏️ Hozirgi:\n<code>{c['card_number']} | {c['holder']} | {c['bank'] or '—'}</code>\n\nYangi:")
    except Exception as e: logger.error(f"card_edit error: {e}")
    await cb.answer()
...
async def card_del(cb: CallbackQuery):
    ...
    except Exception as e: logger.error(f"card_del error: {e}")
    await cb.answer()
...
async def admin_live(cb: CallbackQuery):
    ...
    try:
        ...
        except: await cb.message.answer(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_live error: {e}")
    await cb.answer()
...
async def admin_settings(cb: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_settings error: {e}")
    await cb.answer()
...
async def admin_certs(cb: CallbackQuery):
    ...
    try:
        ...
        b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
        await cb.message.edit_text(f"📜 Sertifikatlar: <b>{cnt}</b>", reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_certs error: {e}")
    await cb.answer()
...
async def admin_battles(cb: CallbackQuery):
    ...
    try:
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("SELECT id, battle_code, status FROM battles ORDER BY created_at DESC LIMIT 10")
        text = "⚔️ <b>BATTLES</b>\n\n"
        for b in rows: text += f"• <code>{b['battle_code']}</code> — {b['status']}\n"
        if not rows: text += "Bo‘sh"
        b = InlineKeyboardBuilder(); b.button(text="⬅️", callback_data="admin:menu")
        await cb.message.edit_text(text, reply_markup=b.as_markup())
    except Exception as e: logger.error(f"admin_battles error: {e}")
    await cb.answer()

@dp.callback_query(F.data == "admin:broadcast")
async def admin_broadcast(cb: CallbackQuery):
    if not await is_admin(cb.from_user.id): return
    await cb.message.edit_text("📢 Broadcast: <code>/broadcast matn</code>")
    await cb.answer()
...
async def cmd_broadcast(message: types.Message):
    ...
    try:
        ...
        if len(parts) < 2: await message.answer("Format: /broadcast matn"); return
        text = parts[1]
        async with db_pool.acquire() as conn:
...
async def handle_admin_text(message: types.Message):
    ...
    if not state: return
    if not await is_admin(uid): ADMIN_STATE.pop(uid, None); return
    ...
    if action == "set_price":
        ...
        except: await message.answer("❌ Noto‘g‘ri")
        return
    if action == "live_set":
        try:
            val = int(message.text.strip())
    ...
    if action == "card_add":
        try:
            ...
            async with db_pool.acquire() as conn:
                await conn.execute("""INSERT INTO payment_cards (card_number, holder, bank, active)
                    VALUES ($1, $2, $3, TRUE)""", parts[0], parts[1], parts[2] if len(parts) > 2 else "")
            ADMIN_STATE.pop(uid, None)
            await message.answer("✅ Karta qo‘shildi")
        except: await message.answer("❌ Format xato")
        return
    if action == "card_edit":
        try:
            parts = [p.strip() for p in message.text.split("|")]
            ...
            cid = state["id"]
            async with db_pool.acquire() as conn:
                await conn.execute("UPDATE payment_cards SET card_number=$1, holder=$2, bank=$3 WHERE id=$4",
                    parts[0], parts[1], parts[2] if len(parts) > 2 else "", cid)
            ADMIN_STATE.pop(uid, None)
