...
async def pay_ok(callback: CallbackQuery):
    ...
    if row["product_code"] == "battle":
        ...
        await bot.send_message(
            ...
            "🏆 Natijangiz ochildi. Endi testni boshlashingiz mumkin.",
            reply_markup=main_menu()
        )
...
def validate_init_data(init_data: str):
    ...
    try:
        ...
        if not received_hash: raise ValueError()
        import urllib.parse
        data={k:urllib.parse.unquote(v) for k,v in data.items()}
        check="\n".join(f"{k}={data[k]}" for k in sorted(data))
        secret=hmac.new(b"WebAppData", BOT_TOKEN.encode(), hashlib.sha256).digest()
        calc=hmac.new(secret,check.encode(),hashlib.sha256).hexdigest()
        if not hmac.compare_digest(calc,received_hash): raise ValueError()
        if "user" not in data: raise ValueError()
...
async def api_config(x_telegram_init_data: str = Header(default="")):
    ...
    return {"products":[dict(r) for r in rows], "bot_username": BOT_USERNAME}
...
async def api_test_submit(payload: dict, x_telegram_init_data: str = Header(default="")):
    ...
    total=int(payload.get("total",18))
    raw=int(payload.get("raw_score",0))
    if test_type=="iq":
        if not await has_access(tg["id"],"iq"):
            raise HTTPException(402,"Payment required")
    score=max(70,min(145,70+round((raw/max(total,1))*75)))
    p=await db()
    await p.execute("""
      INSERT INTO test_results(user_id,test_type,score,raw_score,total,profile)
      VALUES($1,$2,$3,$4,$5,$6::jsonb)
    """,tg["id"],test_type,score,raw,total,json.dumps(profile))
    cert_code = None
    if test_type == "iq":
        cert_code = await create_iq_certificate(tg["id"], profile.get("fullName") or tg.get("first_name") or "Foydalanuvchi", score)
        try:
            cert_path = make_certificate(profile.get("fullName") or tg.get("first_name") or "Foydalanuvchi", score, cert_code)
            await bot.send_document(
                tg["id"],
                BufferedInputFile(open(cert_path, "rb").read(), filename=f"{cert_code}.png"),
    ...
    if battle_id and test_type == "iq":
        ...
        if b and tg["id"] in (b["player1"], b["player2"]):
            if b["status"] == "ready":
                # Both players are considered complete when each has a latest IQ result after battle creation.
                r1 = await p.fetchrow("""
                ...
                if r1 and r2:
                    ...
                    await p.execute("UPDATE battles SET status='finished',result=$1::jsonb WHERE id=$2", json.dumps(result), b["id"])
                    ...
                    for uid, myscore, oppscore, oppid in [
                        ...
                        if winner == uid:
                            ...
                            try:
                                ...
                                await p.execute("INSERT INTO certificates(code,user_id,cert_type,data) VALUES($1,$2,'battle',$3::jsonb)",
                                    vcode,uid,json.dumps({"name":profile.get("fullName") or "Foydalanuvchi","iq":myscore,"opponent":opponent["first_name"] if opponent else "Do‘st","opponent_iq":oppscore}))
                                path=make_certificate(profile.get("fullName") or "Foydalanuvchi",myscore,vcode,True,opponent["first_name"] if opponent else "Do‘st",oppscore)
                                await bot.send_document(uid,BufferedInputFile(open(path,"rb").read(),filename=f"{vcode}.png"),
                                    caption=f"🏆 Battle g‘alaba sertifikatingiz\n🔑 <code>{vcode}</code>")
                            except Exception:
                                pass
                        elif winner is None:
                            ...
                            await bot.send_message(uid, f"⚔️ <b>BATTLE YAKUNLANDI!</b>\n\nSiz: <b>{myscore}</b>\nDo‘stingiz: <b>{oppscore}</b>\n\n🏆 G‘olib: do‘stingiz.")
    return {"score":score,"raw_score":raw,"total":total,"certificate_code":cert_code,"battle_finished":battle_finished}
