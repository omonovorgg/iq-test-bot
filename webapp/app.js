        ...
        puzzle: `
            <svg viewBox="0 0 360 180"
                ...
                <text x="42"
                      ...
                      font-size="15"
                      font-weight="800"
                      fill="currentColor">QIZIL</text>

                <text x="158"
                      y="96"
                      font-size="15"
                      font-weight="800"
                      fill="currentColor">KO‘K</text>

                <text x="262"
                      y="96"
                      font-size="15"
                      font-weight="800"
                      fill="currentColor">YASHIL</text>
            </svg>
        `,
        options: {
...
function updateBackButton() {
    ...
    button.style.visibility =
        state.screen === "home" ? "hidden" : "visible";
}
...
function renderTest() {
    ...
    const puzzle =
        question.puzzle
            ? `<div class="puzzle">${question.puzzle}</div>`
            : "";
            ...
            .map((option, optionIndex) => {
                return `
                    <button
                        ...
                        aria-label="${LETTERS[optionIndex]}: ${escapeHTML(option)}"
                    >
...
async function submitSelectedAnswer() {
    if (
        ...
        state.session.selected === undefined
    ) {
        return;
    }
    ...
    try {
        ...
        if (!Array.isArray(state.session.answers)) {
            state.session.answers = [];
        }

        state.session.answers.push(selected);
...
function renderResult() {
    ...
    screen.innerHTML = `
        <div class="result-header">
            ...
            <div class="eyebrow">
                ${escapeHTML(tr("finished"))}
            </div>

            <h1>
                ${escapeHTML(tr("yourResult"))}
            </h1>
        ...
        <div class="result-stats">
            ...
            <div>
                <strong>
                    ${escapeHTML(result.correct)}/16
                </strong>

                <span>
                    ${escapeHTML(tr("correct"))}
                </span>
            </div>
...
function showInviteRequirement() {
    ...
    card.innerHTML = `
        ...
        <p style="
            ...
            font-size:14px;
        ">
            ${escapeHTML(tr("certificateRequired"))}
        </p>
        ...
        >
            👥
            <span>${escapeHTML(tr("invite"))}</span>
            <span class="arrow">→</span>
        </button>
...
function shareResult() {
    ...
    const text =
        ...
        `${state.result.correct}/16 • ${formatTime(state.result.elapsed)}\n\n` +
        `${tr("shareText")}`;
...
async function loadRanking() {
    ...
    screen.innerHTML = `
        <div class="page-title">
            ...
            <h1>
                ${escapeHTML(tr("ranking"))}
            </h1>

            <p>
                ${escapeHTML(tr("rankingDesc"))}
            </p>
        </div>
...
async function loadProfile() {
    ...
    screen.innerHTML = `
        <div class="page-title">
            ...
            <h1>
                ${escapeHTML(tr("profile"))}
            </h1>

            <p>
                ${escapeHTML(tr("profileDesc"))}
            </p>
        ...
        >
            <div class="stat-card">
                <strong>—</strong>
                <span>${escapeHTML(tr("rank"))}</span>
            </div>

            <div class="stat-card">
                <strong>—</strong>
                <span>${escapeHTML(tr("tests"))}</span>
            </div>

            <div class="stat-card">
                <strong>—</strong>
                <span>${escapeHTML(tr("referrals"))}</span>
            </div>
        </div>
...
function handleBack() {
    ...
    if (
        ...
        const leave =
            window.confirm(
                state.lang === "uz"
                    ? "Testdan chiqmoqchimisiz? Natija saqlanmaydi."
                    : state.lang === "ru"
                        ? "Выйти из теста? Текущий ответ не будет сохранён."
                        : "Leave the test? Your current answer will not be saved."
            );
